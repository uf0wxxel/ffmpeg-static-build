...
if [ "$libpulse_reponse" = "yes" ]; then
	...
	cd pulseaudio-16.1 && \
	PATH="$HOME/bin:$PATH" PKG_CONFIG_PATH="$HOME/ffmpeg_build/lib/pkgconfig:/usr/lib/x86_64-linux-gnu/pkgconfig" meson --prefix="$HOME/ffmpeg_build" --man=false --tests=false build && \
